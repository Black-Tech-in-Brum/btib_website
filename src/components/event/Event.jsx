...
{ datas &&
 datas.map(({meetUps,id, eventTitle,eventDesc,eventDate,eventButton,eventMoreDetailsLink,eventComingSoonLink}) => (
 ...
 <button className=" mt-10 block rounded-lg bg-primary-200  px-12 py-3 font-satoshi font-bold text-white">    
        <a href={eventMoreDetailsLink}> {eventButton}</a>
     </button>
 ...
 <button className=" mt-10 block rounded-lg bg-gray-400  px-12 py-3 font-satoshi font-bold text-white">    
        <a href={eventComingSoonLink}>{eventButton}</a> 
     </button>
 </div> 
   }

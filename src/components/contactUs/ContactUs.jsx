...
function ContactUs() {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_nkht6ve', 'template_90fasgn', form.current, 'cR18-OYcGe0BgyWWs')
      .then((result) => {
  ...
  return (
    <section className="my-6 md:my-10 px-5" id="contact">
      ...
      <form ref= {form} onSubmit={sendEmail} className="max-w-2xl mx-auto my-6 md:my-10 flex flex-col md:flex-row md:flex-wrap justify-center items-center">
        <div className="relative w-full md:w-[48%]">
          <label className="text-neutral-400 text-sm md:text-base font-satoshi absolute mt-3 ml-4" htmlFor="name">Name</label>
          <input id="name" name="user_name" className="py-3 px-4 rounded-lg border border-neutral-400 text-neutral-400 text-sm md:text-base font-satoshi w-full" onClick={hideLabel} onFocus={hideLabel} onBlur={showLabel} required />
        </div>
        <div className="relative my-6 md:my-0 md:ml-6 w-full md:w-[48%]">
          ...
          <input id="email" name="user_email" className="py-3 px-4 rounded-lg border border-neutral-400 text-neutral-400 text-sm md:text-base font-satoshi w-full" onClick={hideLabel} onFocus={hideLabel} onBlur={showLabel} required/>
        </div>
        <div className="relative mb-6 md:my-8 w-full">
          ...
          <textarea id="message" name="message" className="py-3 px-4 rounded-lg border border-neutral-400 text-neutral-400 text-sm md:text-base font-satoshi w-full" rows="10" onClick={hideLabel} onFocus={hideLabel} required/>
        </div>
